...
public class SegTracker {
  ...
  public SegTracker(final Context context, float[] outputs) {
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    for (int i=0; i<128; ++i){
      for (int j=0; j<128; ++j){
        if (this.outputs[z]>0.2){
          float xy[]=new float[]{(float)i, (float)j};
          getFrameToCanvasMatrix().mapPoints(xy);
          canvas.drawPoint(xy[0], xy[1], boxPaint);
  ...
  public synchronized void onFrame(
      ...
      frameWidth = w;
      frameHeight = h;
      this.sensorOrientation = 0;//sensorOrienation;
      initialized = true;
